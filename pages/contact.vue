          ...
          <form name="contact-form">
            <div class="row">
              <div class="col-md-6" v-bind:class="{ 'fld-error': $v.form.name.$error }">
                ...
                <span class="msg-error" v-if="!$v.form.name.required"><small>Поле обязательное для заполнения</small></span>
                <span class="msg-error" v-if="!$v.form.name.minLength"><small>Должно быть не меньше {{ $v.form.name.$params.minLength.min }}</small></span>
              </div>
              <div class="col-md-6" v-bind:class="{ 'fld-error': $v.form.email.$error }">
                ...
                <span class="msg-error" v-if="!$v.form.email.required"><small>Поле обязательное для заполнения</small></span>
                <span class="msg-error" v-if="!$v.form.email.minLength"><small>Невалидный email</small></span>
              </div>
            </div>
            <div class="row mt-3" >
              <div class="col-md-12" v-bind:class="{ 'fld-error': $v.form.subject.$error }">
                ...
                <span class="msg-error" v-if="!$v.form.subject.required"><small>Поле обязательное для заполнения</small></span>
                <span class="msg-error" v-if="!$v.form.subject.minLength"><small>Должно быть не меньше {{ $v.form.subject.$params.minLength.min }}</small></span>
              </div>
            </div>
            <div class="row mt-3">
              <div class="col-md-12" v-bind:class="{ 'fld-error': $v.form.message.$error }">
                <div class="md-form">
                  ...
                  <textarea id="message" rows="2" class="form-control md-textarea" placeholder="Ваше сообщение" v-model="form.message" @input="$v.form.message.$touch()"></textarea>
                </div>
                <span class="msg-error" v-if="!$v.form.message.required"><small>Поле обязательное для заполнения</small></span>
              </div>
            </div>
          </form>
