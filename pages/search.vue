...
export default {
  ...
  async asyncData({route}) {
    ...
    const {data} = await axios.get(encodeURI(`http://127.0.0.1:8000/api/posts/${page_and_search}`));
    // const {data} = await axios.get(`http://127.0.0.1:8000/api/posts/?q=${encodeURIComponent(route.query.q)}`);
